...
// BSS
extern_s s16 sSelectionFlags;
extern_s s16 sCameraSoundFlags;
extern_s u16 sCButtonsPressed;
extern_s struct PlayerCameraState gPlayerCameraState[2];
extern_s struct LakituState gLakituState;
extern_s s16 gCameraMovementFlags;
extern_s s32 gObjCutsceneDone;
extern_s struct Camera *gCamera;
#endif

extern_s struct Object *gCutsceneFocus;
extern_s struct Object *gSecondCameraFocus;
extern_s u8 gRecentCutscene;

// TODO: sort all of this extremely messy shit out after the split

extern_s void set_camera_shake_from_hit(s16 shake);
extern_s void set_environmental_camera_shake(s16 shake);
extern_s void set_camera_shake_from_point(s16 shake, f32 posX, f32 posY, f32 posZ);
extern_s void move_mario_head_c_up(UNUSED struct Camera* c);
extern_s void transition_next_state(UNUSED struct Camera* c, s16 frames);
extern_s void set_camera_mode(struct Camera* c, s16 mode, s16 frames);
extern_s void update_camera(struct Camera* c);
extern_s void reset_camera(struct Camera* c);
extern_s void init_camera(struct Camera* c);
extern_s void select_mario_cam_mode(void);
extern_s Gfx* geo_camera_main(s32 callContext, struct GraphNode* g, void* context);
extern_s void stub_camera_2(UNUSED struct Camera* c);
extern_s void stub_camera_3(UNUSED struct Camera* c);
extern_s void vec3f_sub(Vec3f dst, Vec3f src);
extern_s void object_pos_to_vec3f(Vec3f dst, struct Object* o);
extern_s void vec3f_to_object_pos(struct Object* o, Vec3f src);
extern_s s32 move_point_along_spline(Vec3f p, struct CutsceneSplinePoint spline[], s16* splineSegment, f32* progress);
extern_s s32 cam_select_alt_mode(s32 angle);
extern_s s32 set_cam_angle(s32 mode);
extern_s void set_handheld_shake(u8 mode);
extern_s void shake_camera_handheld(Vec3f pos, Vec3f focus);
extern_s s32 find_c_buttons_pressed(u16 currentState, u16 buttonsPressed, u16 buttonsDown);
extern_s s32 update_camera_hud_status(struct Camera* c);
extern_s s32 collide_with_walls(Vec3f pos, f32 offsetY, f32 radius);
extern_s s32 clamp_pitch(Vec3f from, Vec3f to, s16 maxPitch, s16 minPitch);
extern_s s32 is_within_100_units_of_mario(f32 posX, f32 posY, f32 posZ);
extern_s s32 set_or_approach_f32_asymptotic(f32* dst, f32 goal, f32 scale);
extern_s s32 approach_f32_asymptotic_bool(f32* current, f32 target, f32 multiplier);
extern_s f32 approach_f32_asymptotic(f32 current, f32 target, f32 multiplier);
extern_s s32 approach_s16_asymptotic_bool(s16* current, s16 target, s16 divisor);
extern_s s32 approach_s16_asymptotic(s16 current, s16 target, s16 divisor);
extern_s void approach_vec3f_asymptotic(Vec3f current, Vec3f target, f32 xMul, f32 yMul, f32 zMul);
extern_s void set_or_approach_vec3f_asymptotic(Vec3f dst, Vec3f goal, f32 xMul, f32 yMul, f32 zMul);
extern_s s32 camera_approach_s16_symmetric_bool(s16* current, s16 target, s16 increment);
extern_s s32 set_or_approach_s16_symmetric(s16* current, s16 target, s16 increment);
extern_s s32 camera_approach_f32_symmetric_bool(f32* current, f32 target, f32 increment);
extern_s f32 camera_approach_f32_symmetric(f32 value, f32 target, f32 increment);
extern_s void random_vec3s(Vec3s dst, s16 xRange, s16 yRange, s16 zRange);
extern_s s32 clamp_positions_and_find_yaw(Vec3f pos, Vec3f origin, f32 xMax, f32 xMin, f32 zMax, f32 zMin);
extern_s s32 is_range_behind_surface(Vec3f from, Vec3f to, struct Surface* surf, s16 range, s16 surfType);
extern_s void scale_along_line(Vec3f dest, Vec3f from, Vec3f to, f32 scale);
extern_s s16 calculate_pitch(Vec3f from, Vec3f to);
extern_s s16 calculate_yaw(Vec3f from, Vec3f to);
extern_s void calculate_angles(Vec3f from, Vec3f to, s16* pitch, s16* yaw);
extern_s f32 calc_abs_dist(Vec3f a, Vec3f b);
extern_s f32 calc_hor_dist(Vec3f a, Vec3f b);
extern_s void rotate_in_xz(Vec3f dst, Vec3f src, s16 yaw);
extern_s void rotate_in_yz(Vec3f dst, Vec3f src, s16 pitch);
extern_s void set_camera_pitch_shake(s16 mag, s16 decay, s16 inc);
extern_s void set_camera_yaw_shake(s16 mag, s16 decay, s16 inc);
extern_s void set_camera_roll_shake(s16 mag, s16 decay, s16 inc);
extern_s void set_pitch_shake_from_point(s16 mag, s16 decay, s16 inc, f32 maxDist, f32 posX, f32 posY, f32 posZ);
extern_s void shake_camera_pitch(Vec3f pos, Vec3f focus);
extern_s void shake_camera_yaw(Vec3f pos, Vec3f focus);
extern_s void shake_camera_roll(s16* roll);
extern_s s32 offset_yaw_outward_radial(struct Camera* c, s16 areaYaw);
extern_s void play_camera_buzz_if_cdown(void);
extern_s void play_camera_buzz_if_cbutton(void);
extern_s void play_camera_buzz_if_c_sideways(void);
extern_s void play_sound_cbutton_up(void);
extern_s void play_sound_cbutton_down(void);
extern_s void play_sound_cbutton_side(void);
extern_s void play_sound_button_change_blocked(void);
extern_s void play_sound_rbutton_changed(void);
extern_s void play_sound_if_cam_switched_to_lakitu_or_mario(void);
extern_s s32 radial_camera_input(struct Camera* c, UNUSED f32 unused);
extern_s s32 trigger_cutscene_dialog(s32 trigger);
extern_s void handle_c_button_movement(struct Camera* c);
extern_s void start_cutscene(struct Camera* c, u8 cutscene);
extern_s u8 get_cutscene_from_mario_status(struct Camera* c);
extern_s void warp_camera(f32 displacementX, f32 displacementY, f32 displacementZ);
extern_s void approach_camera_height(struct Camera* c, f32 goal, f32 inc);
extern_s void offset_rotated(Vec3f dst, Vec3f from, Vec3f to, Vec3s rotation);
extern_s s16 next_lakitu_state(Vec3f newPos, Vec3f newFoc, Vec3f curPos, Vec3f curFoc, Vec3f oldPos, Vec3f oldFoc, s16 yaw);
extern_s void set_fixed_cam_axis_sa_lobby(UNUSED s16 preset);
extern_s s16 camera_course_processing(struct Camera* c);
extern_s void resolve_geometry_collisions(Vec3f pos, UNUSED Vec3f lastGood);
extern_s s32 rotate_camera_around_walls(struct Camera* c, Vec3f cPos, s16* avoidYaw, s16 yawRange);
extern_s void find_mario_floor_and_ceil(struct PlayerGeometry* pg);
extern_s u8 start_object_cutscene_without_focus(u8 cutscene);
extern_s s16 cutscene_object_with_dialog(u8 cutscene, struct Object* o, s16 dialogID);
extern_s s16 cutscene_object_without_dialog(u8 cutscene, struct Object* o);
extern_s s16 cutscene_object(u8 cutscene, struct Object* o);
extern_s void play_cutscene(struct Camera* c);
extern_s s32 cutscene_event(CameraEvent event, struct Camera* c, s16 start, s16 end);
extern_s s32 cutscene_spawn_obj(u32 obj, s16 frame);
extern_s void set_fov_shake(s16 amplitude, s16 decay, s16 shakeSpeed);

extern_s void set_fov_function(u8 func);
extern_s void cutscene_set_fov_shake_preset(u8 preset);
extern_s void set_fov_shake_from_point_preset(u8 preset, f32 posX, f32 posY, f32 posZ);
extern_s void obj_rotate_towards_point(struct Object* o, Vec3f point, s16 pitchOff, s16 yawOff, s16 pitchDiv, s16 yawDiv);

extern_s Gfx* geo_camera_fov(s32 callContext, struct GraphNode* g, UNUSED void* context);
