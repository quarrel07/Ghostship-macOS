...
static UIWidgets::ComponentAlignments MapAlign(C_ComponentAlign align) {
    if (align == C_ALIGN_RIGHT) return UIWidgets::ComponentAlignments::Right;
    return UIWidgets::ComponentAlignments::Left;
}

static UIWidgets::LabelPositions MapLabel(C_LabelPos pos) {
    if (pos == C_LABEL_FAR) return UIWidgets::LabelPositions::Far;
...
extern "C" HM_API void C_AddWidget(const char* section, const char* sidebar, int column, const char* label, C_WidgetConfig* config) {
    ...
    if (config->cvar) widget.CVar(config->cvar);
    if (config->window_name) widget.WindowName(config->window_name);
    ...
    switch (config->type) {
        ...
        case C_WIDGET_CVAR_SLIDER_INT: {
            ...
            if (config->opts.slider_int.format) opt.Format(config->opts.slider_int.format);
            widget.Options(opt);
            break;
        }
        ...
        case C_WIDGET_CVAR_SLIDER_FLOAT: {
            ...
            if (config->opts.slider_float.format) opt.Format(config->opts.slider_float.format);
            widget.Options(opt);
            break;
        }
        ...
        case C_WIDGET_CVAR_COLOR_PICKER: {
            ...
            widget.Options(UIWidgets::ColorPickerOptions()
                .Tooltip(config->opts.color_picker.tooltip ? config->opts.color_picker.tooltip : "")
        ...
        case C_WIDGET_WINDOW_BUTTON: {
            widget.Options(UIWidgets::WindowButtonOptions()
                .Tooltip(config->opts.generic.tooltip ? config->opts.generic.tooltip : "")
            );
            break;
        }
        ...
        // --- DEFAULT FALLBACK ---
        default: {
            widget.Options(UIWidgets::WidgetOptions()
                .Tooltip(config->opts.generic.tooltip ? config->opts.generic.tooltip : "")
            );
            break;
        }
