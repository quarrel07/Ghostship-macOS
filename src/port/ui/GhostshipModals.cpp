...
std::vector<GhostshipModal> modals;

bool closePopup = false;

void GhostshipModalWindow::Draw() {
    if (!IsVisible()) {
        return;
    }
    DrawElement();
...
void GhostshipModalWindow::DrawElement() {
    if (modals.size() > 0) {
        GhostshipModal curModal = modals.at(0);
...
void GhostshipModalWindow::DismissPopup() {
    closePopup = true;
}
